...
def main() -> None:
    ...
    frame_areas = [
        int((predictions[:, :, :, frame_idx - 1] == args.target_class).sum())
        for frame_idx in frame_indices
    ]
    ed_frame = int(np.argmax(frame_areas)) + 1
    es_frame = int(np.argmin(frame_areas)) + 1

    csv_path = args.output_dir / "frame_areas.csv"
    ...
    plt.title(f"Area curve for {CLASS_NAMES.get(args.target_class, args.target_class)}")
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(args.output_dir / "area_curve.png", dpi=180)
    plt.close()
    ...
    summary = {
        ...
        "estimated_max_frame_1_based": ed_frame,
        "estimated_min_frame_1_based": es_frame,
        "artifacts": {
            "pred_masks_4d": str(args.output_dir / "pred_masks_4d.nii.gz"),
            "frame_areas_csv": str(csv_path),
            "area_curve_png": str(args.output_dir / "area_curve.png"),
        },
    }
    if "ED" in info_cfg:
